...
export const WideBanner = ({
  ...
  return (
    <div
      ...
      className={`cu-widebanner relative flex items-center justify-center mx-auto px-8 overflow-hidden md:px-16 rounded-xl not-prose ${$componentMarginY} ${hasImageStyles} ${paddingY}`}
    >
      {hasWave && (
        <svg
      ...
      {image && isType === 'image' && (
        <div className="absolute top-0 w-full h-screen bg-black" style={opacityStyle}></div>
      )}

      <div
        className={`relative z-[2] max-w-4xl w-full flex flex-col items-center gap-2 cu-widebanner-${isType} cu-zero-first-last`}
      >
        <PageHeader
          header={title}
          as={headerType}
