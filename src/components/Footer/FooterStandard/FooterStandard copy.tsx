...
export const FooterStandard = () => {
  return (
    <footer
      className={`cu-footerstandard not-prose bg-cu-black-900 ${footerStyles.wave}`}
    ...
    >
      ...
      <div className="mx-auto flex max-w-7xl flex-col-reverse gap-1 p-8 text-sm text-cu-black-300 sm:gap-4 sm:pb-8 sm:text-lg lg:grid-cols-3 lg:flex-row lg:justify-between lg:text-left lg:text-sm">
        ...
        <ul className="mb-4 flex justify-center space-x-6 lg:mb-0 lg:justify-end">
          {Social.map((item, index) => (
            <li key={index}>
              <Link href={item.href} className="text-gray-200 hover:text-cu-red">
                <span className="sr-only">{item.name}</span>
                {/* TODO fix class on icon or update icon setup */}
